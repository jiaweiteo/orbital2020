...
export default class CoupleOverview extends React.Component {
    static navigationOptions = {
        title: 'Couple Overview',
    };

    constructor(props) {
    super(props);
      
    this.itemH = trackCouple.state.itemH
    this.costH = trackCouple.state.costH
    this.catH = trackCouple.state.catH
    this.history = trackCouple.state.history
    this.array = [],
  ...
  promptUser(key)  {
    ...
    const buttons = [
        { text: 'No', onPress: () => {} },
        { text: 'Yes', onPress: () => {
          const storeAll = trackCouple.deleteItem(key)
  ...
  render() {
    ...
    const budgetMonth = trackCouple.getBudgetMonth();
    ...
    return (
        <View style={styles.MainContainer}>       
          <LinearGradient
            colors={['rgba(255,100,200,0.8)', 'transparent']}
            style={{
              position: 'absolute',
              left: 0,
              right: 0,
...
const styles = StyleSheet.create({
  ...
  footer: {
    backgroundColor: 'rgba(251, 156, 221, 0.3)',
    flex: 1,
    borderTopLeftRadius: 30,
    borderTopRightRadius: 30,
    paddingVertical: 10,
