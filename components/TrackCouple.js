...
class TrackCouple extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
          user: "",
      ...
      addPortfolio() {
        ...
        checkMonth.collection("Couple Expense").doc('Expenses').set({
            expenses: this.state.expenses,
        }).then();
        checkMonth.collection("Couple Expense").doc('Items').set({
            items: this.state.itemH
        }).then();
        checkMonth.collection("Couple Expense").doc('Cost').set({
            cost: this.state.costH
        }).then();
        checkMonth.collection("Couple Expense").doc('Category').set({
            category: this.state.catH
        }).then();

        const storeForCouple = firebase.firestore().collection("users").doc(this.state.coupleUID)

        storeForCouple.collection("Couple Expense").doc('Expenses').set({
          expenses: this.state.expenses,
        }).then();
        storeForCouple.collection("Couple Expense").doc('Items').set({
            items: this.state.itemH
        }).then();
        storeForCouple.collection("Couple Expense").doc('Cost').set({
            cost: this.state.costH
        }).then();
        storeForCouple.collection("Couple Expense").doc('Category').set({
            category: this.state.catH
        }).then();
    }
      ...
      updateUserDetails(items, cost, category, budget, expenses, date, month, id, coupleUID) {
        ...
        this.state.coupleUID = coupleUID
        const tempHistory = []
      ...
      updateCurrentBudget(money) {
        ...
        docRef.collection("Couple Expense").doc("Budget").set({
          budget: this.state.budget,
        }).then();

        const storeForCouple = firebase.firestore().collection("users").doc(this.state.coupleUID)

        storeForCouple.collection("Couple Expense").doc("Budget").set( {
          budget: this.state.budget
        }).then()
      ...
      updateBudget(money, datee) {
        if (this.state.date == "") {
          ...
          docRef.collection("Couple Expense").doc('Date').set({
          ...
          storeForCouple.collection("Couple Expense").doc('Date').set({
            date: this.state.date,
            month: this.state.month,
            id: datee
          }).then();
        ...
        const user = firebase.auth().currentUser;
        const docRef = firebase.firestore().collection("users").doc(user.uid)
        const past = docRef.collection("Couple Past Budget").doc("IDs")
        const id = this.state.dateID
        let tempArr = []
        let count = 0;
        past.onSnapshot( doc => {
        ...
        const storeForCouple = firebase.firestore().collection("users").doc(this.state.coupleUID)
        const pastCouple = storeForCouple.collection("Couple Past Budget").doc("IDs")

        let tempArr2 = []
        let count2 = 0;
        pastCouple.onSnapshot( doc => {
          tempArr2 = doc.data().temp
          tempArr2.push(id)
          if (count2 == 0) {
            pastCouple.set({
              temp: tempArr2
        ...
        pastCouple.collection(id).doc('Budget').set({
          budget: this.state.budget,
        }).then();
        pastCouple.collection(id).doc('Expenses').set({
          expenses: this.state.expenses,
        }).then();
        pastCouple.collection(id).doc('LeftOver').set({
            leftover: this.state.budget - this.state.expenses
        }).then();
        pastCouple.collection(id).doc('Items').set({
            items: this.state.itemH
        }).then();
        pastCouple.collection(id).doc('Cost').set({
            cost: this.state.costH
        }).then();
        pastCouple.collection(id).doc('Category').set({
            category: this.state.catH
        })
        pastCouple.collection(id).doc('Date').set({
          date: this.state.date,
          month: this.state.month,
          id: id
        }).then();
        ...
        const numdd = Number(datee.substring(0,2))
        const monthNumber = Number(datee.substring(3,5))
        this.updateUserDetails([], [], [], Number(money), 0, numdd, months[monthNumber - 1], datee, this.state.coupleUID)
        this.addPortfolio()
        docRef.collection("Couple Expense").doc('Budget').set({
          budget: this.state.budget
        }).then()
        ...
        docRef.collection("Couple Expense").doc('LeftOver').set({
        ...
        storeForCouple.collection("Couple Expense").doc('LeftOver').set({
          leftover: this.state.budget
        }).then()
...
const trackCouple = new TrackCouple()
export default trackCouple
